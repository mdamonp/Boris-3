...
static char gTranslationLogPath[MAX_PATHNAME_LEN];
			...
			switch(gsSystem.panel[panelIndex].station[stationIndex].state)
			{
				...
				case B3_STATION_STATE_STABILIZING_EACG:
					if (lastState[panelIndex][stationIndex] != gsSystem.panel[panelIndex].station[stationIndex].state)
					{
						...
						{
							baseTimeForStabilizationToCompleteInSec[panelIndex][stationIndex] = Timer();
						}
					...
					if ((gsSystem.panel[panelIndex].station[stationIndex].state == B3_STATION_STATE_STABILIZING) &&
						...
						(deltaTimeInSec > gsSystem.panel[panelIndex].procedure.maxTimeToStabilizationInSeconds))
					{
				...
				case B3_STATION_STATE_READY_FOR_RESIN:
					...
					if (!alertControlVisible)
					{
						...
						gsSystem.panel[panelIndex].station[stationIndex].state = B3_STATION_STATE_WAITING_FOR_TRANSLATION_READY;
						break;
					}
				...
				case B3_STATION_STATE_WAITING_FOR_TRANSLATION_READY_EACG:
					if (lastState[panelIndex][stationIndex] != gsSystem.panel[panelIndex].station[stationIndex].state)
					{
						...
						baseTimeForTranslationReadyInSec[panelIndex][stationIndex] = Timer();
...
//*/
					...
					switch(gsSystem.panel[panelIndex].procedure.translationDetectionMethod)
					{
						case B3_TRANSLATE_DETECTION_PEAK_METHOD:
							if ((gsSystem.panel[panelIndex].station[stationIndex].state == B3_STATION_STATE_WAITING_FOR_TRANSLATION_READY) &&
								...
								(deltaTimeInSec > gsSystem.panel[panelIndex].procedure.peakTemperatureTranslationDetectionMethodInfo.maxTimeForThresholdDetectionInSeconds))
							{
				...
				case B3_STATION_STATE_READY_FOR_TRANSLATION_EACG:
					if (lastState[panelIndex][stationIndex] != gsSystem.panel[panelIndex].station[stationIndex].state)
					{
						...
						if ((gsSystem.control.translatorInPlace) &&
						    ...
						    (gsSystem.translateInfo.state == GALIL_CMD_TRANSLATE_READY))
						{
							B3SystemLogWithVargs(gsAppLogPath, "Translation automatically started alert posted for panel %d - station %d",
												panelIndex+1, stationIndex+1);
							gsSystem.panel[panelIndex].station[stationIndex].state = B3_STATION_STATE_TRANSLATING;
							break;
						}
						else
						{
					...
					else
					{
							...
							(gsSystem.panel[panelIndex].procedure.peakTemperatureTranslationDetectionMethodInfo.percentTempDropForThresholdDetectionError > 0.0))
						{
							...
							if (anchorTempInF < (peakTempInF[panelIndex][stationIndex] - tempDropforTranErrorDetectInF))
							{
								ctrlArrayHnd = GetCtrlArrayFromResourceID(gsB3PanelPnlHndl[panelIndex], STATUS_ALERT_CA);
								controlId = GetCtrlArrayItem (ctrlArrayHnd, stationIndex);
								SetCtrlAttribute(gsB3PanelPnlHndl[panelIndex], controlId, ATTR_VISIBLE, 0);
							...
							(gsSystem.panel[panelIndex].procedure.fixedTimeTranslationDetectionMethodInfo.maxTimeForTranslationReadyAckInSeconds > 0.0))
						{
							...
							if (startDelayTimeInSec >
								gsSystem.panel[panelIndex].procedure.fixedTimeTranslationDetectionMethodInfo.maxTimeForTranslationReadyAckInSeconds)
							{
								ctrlArrayHnd = GetCtrlArrayFromResourceID(gsB3PanelPnlHndl[panelIndex], STATUS_ALERT_CA);
								controlId = GetCtrlArrayItem (ctrlArrayHnd, stationIndex);
								SetCtrlAttribute(gsB3PanelPnlHndl[panelIndex], controlId, ATTR_VISIBLE, 0);
				...
				case B3_STATION_STATE_TRANSLATING:
					if (lastState[panelIndex][stationIndex] != gsSystem.panel[panelIndex].station[stationIndex].state)
					{
						...
						translationComplete[panelIndex][stationIndex] = 0;
						...
						strcpy(gTranslationLogPath, gsLogPath[panelIndex][stationIndex]);
						strcat(gTranslationLogPath, ".tlg");
					...
					if (translationComplete[panelIndex][stationIndex] == 0)
					{
						if ((gsSystem.translateInfo.state == GALIL_CMD_TRANSLATE_COMPLETE) || (gsSystem.translateInfo.state == GALIL_CMD_TRANSLATE_COMPLETE_WITH_LIMIT_TRIP))
						{
							gsSystem.panel[panelIndex].station[stationIndex].state = B3_STATION_STATE_TRANSLATING_COMPLETE;
							break;
						}
					}
...
//							if (gsSystem.translateInfo.loadInLBS > gsSystem.control.simTranslatorData.safeMaxLoadInLBS)
				...
				case B3_STATION_STATE_READY_FOR_HEATING:
					...
					for (int i=0; i<B3_MAX_STATION_COUNT; i++)
					{
						...
						{
							if (gsSystem.panel[panelIndex].station[i].state == B3_STATION_STATE_OFF)
							{
								continue;
							}
							else if ((gsSystem.panel[panelIndex].station[i].state < B3_STATION_STATE_OFF) ||
									 (gsSystem.panel[panelIndex].station[i].state >= B3_STATION_STATE_STABILIZING_EACG))
							{
								readyForHeating = 0;
								break;
							}
							else if (gsSystem.panel[panelIndex].station[i].state < B3_STATION_STATE_READY_FOR_HEATING)
							{
								readyForHeating = 0;
								break;
							}
							else if (gsSystem.panel[panelIndex].station[i].state == B3_STATION_STATE_HEATING)
							{
								readyForHeating = 1;
								break;
							}
					...
					if (readyForHeating)
					{
						gsSystem.panel[panelIndex].station[stationIndex].state = B3_STATION_STATE_HEATING;
					}
