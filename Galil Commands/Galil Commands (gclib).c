...
int DLLEXPORT DLLSTDCALL GALIL_CMD_Open(GCStringIn controllerId)
{
	...
	long int dmcStat = GOpen(controllerId, &gsDmcHnd);  
	
	if(dmcStat != G_NO_ERROR)    // dmcStat equal zero if all went well
	{
		GALIL_CMD_QUEUE_ERROR_INFO_VAR_ARGS(GALIL_CMD_LOCK_ERROR, "Unable to perform DMCOpen for controller ID = %s - dmcStat Error = %d", controllerId, dmcStat);
...
int DLLEXPORT DLLSTDCALL GALIL_CMD_Close(void)
{
	...
	if (gsDmcHnd != 0)
	{
		long int dmcStat = GClose(gsDmcHnd); 
...
int DLLEXPORT DLLSTDCALL GALIL_CMD_StatusCmd(int* const state,  
									double* const aPosInInches, 
									double* const loadInPounds,
									const double timeout, int* const cmdStat)
{
	if (!gsGalilCommOpenned)
	{
		GALIL_CMD_QUEUE_ERROR_INFO(GALIL_CMD_LOCK_ERROR, "Connection to Galil not established");
	...
	int dmcCmd = READ_STATUS_COMMAND;
	
	double dReplyArray[3] = {0.0};
	
	int stat = _GALIL_SendCmdMsg(dmcCmd,
							NULL,
							0,
							dReplyArray,
							sizeof(dReplyArray)/sizeof(double),
							timeout,
							cmdStat);
...
int DLLEXPORT DLLSTDCALL GALIL_CMD_ReadDataRecord(DMCDATARECORDQR const dataRecord, const double timeout, int* const cmdStat)
{
	...
	*gsTlvLockPtr = TRUE;
	...
	do
	{
		...
		if (dmcStat != G_NO_ERROR)
		{
			//EventLogVError(g//EventLogHandle, "GArrayUpload Send Error %d - retry %d", dmcStat, (retryCnt+1));
			...
			{
				//EventLogError(g//EventLogHandle, "GArrayUpload Send Error max retry limit error");
	...
	*gsTlvLockPtr = TRUE;
	...
	do
	{
		...
		do
		{
			...
			long int dmcStat = GCmdI(gsDmcHnd, dmcCmdMsg, &cmdPend); 

			if (dmcStat != G_NO_ERROR)
			{
				//EventLogVError(g//EventLogHandle, "GCmdI( Send Error (%s) %d", dmcCmdMsg, dmcStat);
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogVError(g//EventLogHandle, "GCmdI( Send (%s) max retry limit error", dmcCmdMsg);
					stat = GALIL_CMD_SEND_RETRY_ERROR;
					goto DONE;
				}
	...
	if (argCnt <= 5)
	{
		sprintf(dmcCmdMsg, "CMDARG[0]=%d;CMDARG[1]=%d", cmd, argCnt);

		for (int argIndex=0; argIndex<argCnt; argIndex++)
		{
			sprintf(dmcCmdMsg, "%s;CMDARG[%d]=%d", dmcCmdMsg, argIndex+2, arg[argIndex]);
		}

		sprintf(dmcCmdMsg, "%s;CMDPEND=1", dmcCmdMsg);
		...
		do
		{
			...
			long int dmcStat = GCmdT(gsDmcHnd, dmcCmdMsg, dmcRespMsg, (ULONG)sizeof(dmcRespMsg), NULL); 

			if (dmcStat != G_NO_ERROR)
			{
				//EventLogVError(g//EventLogHandle, "GCmdT( Send Error (%s) %d", dmcCmdMsg, dmcStat);
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogVError(g//EventLogHandle, "GCmdT( Send (%s) max retry limit error", dmcCmdMsg);
					stat = GALIL_CMD_SEND_RETRY_ERROR;
					goto DONE;
				}
	...
	else
	{
		if (argCnt <= MAX_GALIL_ARG_CNT)
		{
			...
			do
			{
				...
				long int dmcStat = GArrayDownload(gsDmcHnd, 
										"CMDARG[]", 
										0, 
										argCnt+1, 
										config); 
				...
				if (dmcStat != G_NO_ERROR)
				{
					//EventLogVError(g//EventLogHandle, "GArrayDownload (CMDARG[]) Send Error %d", dmcStat);
					...
					if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
					{
						//EventLogError(g//EventLogHandle, "GArrayDownload (CMDARG[]) max retry limit error");
						stat = GALIL_CMD_SEND_RETRY_ERROR;
						goto DONE;
					}
	...
	if ((resultCnt+3) <= MAX_GALIL_ARG_CNT)
	{
		...
		do
		{
			...
			long int dmcStat = GArrayUpload(gsDmcHnd, 
									"CMDRES[]", 
									0, 
									resultCnt+3-1,
									G_COMMA,
			...
			if (dmcStat != G_NO_ERROR)
			{
				//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Send Error %d - retry %d", dmcStat, (retryCnt+1));
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogError(g//EventLogHandle, "GArrayDownload (CMDRES[]) max retry limit error");
					stat = GALIL_CMD_SEND_RETRY_ERROR;
					goto DONE;
				}
			...
			if (token == NULL)
			{
				//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse cmd id token Error - retry %d ", (retryCnt+1));
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse cmd id token max retry limit error");
			...
			if (returnCmd != cmd)
			{
				//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) invalid cmd id Error - expected %d - received %d - retry %d", cmd, returnCmd, (retryCnt+1));
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogError(g//EventLogHandle, "GArrayUpload (CMDRES[]) invalid cmd id Error max retry limit error");
					stat = GALIL_CMD_PARSE_RETRY_ERROR;
					goto DONE;
				}
			...
			if (token == NULL)
			{
				//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse cmdStat token Error - retry %d ", (retryCnt+1));
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse cmdStat token max retry limit error");
					stat = GALIL_CMD_PARSE_RETRY_ERROR;
					goto DONE;
				}
			...
			if (token == NULL)
			{
				//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse returnCnt token Error - retry %d ", (retryCnt+1));
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse returnCnt token max retry limit error");
					stat = GALIL_CMD_PARSE_RETRY_ERROR;
					goto DONE;
				}
			...
			{
				//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) invalid resultCnt Error - expected %d - received %d - retry %d", resultCnt, returnCnt, (retryCnt+1));
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogError(g//EventLogHandle, "GArrayUpload (CMDRES[]) invalid resultCnt Error max retry limit error");
					stat = GALIL_CMD_PARSE_RETRY_ERROR;
					goto DONE;
				}
			...
			if (resultCnt > 0)
			{
				for (i=0; i<resultCnt; i++)
				{
					...
					if (token == NULL)
					{
						//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse result[%d] token Error - retry %d", i, (retryCnt+1));
						...
						if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
						{
							//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse result[%d] token max retry limit error", i);
							stat = GALIL_CMD_PARSE_RETRY_ERROR;
							goto DONE;
						}
		...
		while (argRemaining > 0)
		{
			...
			do
			{
				...
				if (dmcStat != G_NO_ERROR)
				{
					...
					if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
					{
						//EventLogError(g//EventLogHandle, "GArrayUpload (CMDRES[]) max retry limit error");
						stat = GALIL_CMD_SEND_RETRY_ERROR;
						goto DONE;
					}
				...
				if (token == NULL)
				{
					//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse result[%d] token Error - retry %d", base, (retryCnt+1));
					...
					if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
					{
						//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[]) Parse result[%d] token max retry limit error", base);
						stat = GALIL_CMD_PARSE_RETRY_ERROR;
						goto DONE;
					}
...
DONE:
	...
	do
	{
		...
		if (dmcStat != G_NO_ERROR)
		{
			//EventLogVError(g//EventLogHandle, "GCmdT( Send Error (%s) %d - retry %d", dmcCmdMsg, dmcStat, (retryCnt+1));
	...
	*gsTlvLockPtr = TRUE;
	...
	do
	{
		...
		for (i=0; i<argCnt; i++)
		{
			if (token == NULL)
			{
				//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[%d]) Parse cmd id token Error - retry %d", i, (retryCnt+1));
				...
				if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
				{
					//EventLogVError(g//EventLogHandle, "GArrayUpload (CMDRES[%d]) Parse cmd id token max retry limit error", i);
					stat = GALIL_CMD_PARSE_RETRY_ERROR;
					goto DONE;
				}
...
//	int stat = CmtGetLock (gsGalilLockHnd);
	...
	*gsTlvLockPtr = TRUE;
	...
	do
	{
		...
		dmcStat = GArrayDownload(gsDmcHnd, 
								arrayName, 
								0, 
								argCnt-1, 
								dmcMsg);
		...
		if (dmcStat != G_NO_ERROR)
		{
			//EventLogVError(g//EventLogHandle, "GArrayDownload Send Error %d - retry %d", dmcStat, (retryCnt+1));
			...
			if (retryCnt >= GALIL_MAX_MSG_RETRY_CNT)
			{
				//EventLogError(g//EventLogHandle, "GArrayDownload Send Error max retry limit error");
				stat = GALIL_CMD_SEND_RETRY_ERROR;
				goto DONE;
			}
